...
export class PiRpcApi {
  ...
  /**
   * User shell command (composer `!` / `!!`).
   * Matches Pi TUI: `!cmd` records bash output in context; `!!cmd` excludes it.
   */
  bash(command: string, options: { excludeFromContext?: boolean } = {}): Promise<void> {
    return this.connection.request({
      type: "bash",
      command,
      ...(options.excludeFromContext ? { excludeFromContext: true } : {}),
    }, 30_000) as Promise<void>;
  }

  abortBash(): Promise<void> {
    return this.connection.request({ type: "abort_bash" } as Parameters<PiRpcConnection["request"]>[0]) as Promise<void>;
  }
