...
- Composer prompt history: **↑ / ↓** recalls previous inputs (bash/Copilot-style), per session.

## [1.0.19] - 2026-09-21

### Changed

- Remove redundant Steer/Queue buttons from the session header; keep the composer ⚡ send-mode control only.

## [1.0.18] - 2026-09-20

### Added

- Conversation header **Steer / Queue** toggle for `piAgent.composer.streamingBehavior` (writes global VS Code setting).

## [1.0.17] - 2026-09-20

### Changed

- Default `piAgent.composer.streamingBehavior` is now **steer** (插队/转向) instead of followUp/Queue.

## [1.0.16] - 2026-09-20

### Changed

- Lazy SessionRuntime: do not instantiate every persisted session on activation; create when opened/started.
- Skip boot-time git worktree reconcile + bulk title repair; welcome catalog uses readdir of the current workspace session dir only (no rg).
- Cache Material icon data-URIs; history deferred until session starts.
...
- Welcome skills/extensions ride on presentation snapshot (registry cache) so the top-bar list cannot stay empty when a webview message is dropped.

## [1.0.13] - 2026-09-19

### Fixed

- Welcome skills list: follow directory symlinks (`~/.agents/skills/ego-browser`) and merge `~/.agents/.skill-lock.json` names.

## [1.0.12] - 2026-09-19

### Changed

- Welcome top bar: left **version / extensions / skills**, right **settings**.
- Session `…` menu slimmed: proxy, diagnostics, refresh-commands moved to welcome settings.
...
- Welcome session `…` menu closes on **Esc** or click outside (no need to toggle `…` again).
- Dismissed rename input no longer commits on blur.

## [1.0.9] - 2026-09-19

### Added

- Welcome session row `…` menu: **重命名会话** / **删除会话**.
- Catalog rename appends Pi `session_info` to the JSONL; delete removes the session file (and closes a live runtime if open).

## [1.0.8] - 2026-09-19

### Changed

- Welcome history list shows **only sessions for the current VS Code folder** (e.g. vscode_log), not all projects under ~/.pi/agent.
- Path filter + workspace catalog fallback so the list still fills when git worktree discovery is empty.

## [1.0.7] - 2026-09-19

### Fixed

- Welcome session list rendered empty because Svelte `$derived(() => …)` stored a function, not an array. Use `$derived.by`.
- Refresh Pi catalog sessions when the sidebar attaches / registry changes.

## [1.0.6] - 2026-09-19

### Changed

- Welcome session list includes recent Pi sessions from all workspaces under ~/.pi/agent (like a global history), not only the current folder.

## [1.0.5] - 2026-09-19

### Fixed

- Welcome session list now includes on-disk Pi sessions (`~/.pi/agent`) for the workspace, not only live runtimes.
- Open a historical session directly from the welcome list.

## [1.0.4] - 2026-09-19

### Fixed

- Welcome view receives the session list from the sidebar presentation store.
...
- Scale the Pi brand mark to about 75% of the rounded icon tile.

## [1.0.1] - 2026-09-19

### Changed

- Enlarge the Pi brand mark in the marketplace icon and Activity Bar tile so it reads clearly at small sizes.

## [1.0.0] - 2026-09-19

### Changed

- **Rebrand** Frost UI / FrostPi → **Pi Coding Agent** (sidebar label: **Pi**).
- Extension ID: `yesho.frostui` → **`yesho.pi-coding-agent`**.
- Settings prefix: `frostui.*` → **`piAgent.*`**.
- Commands and views: `frostui.*` → **`piAgent.*`**; command titles now use **Pi Coding Agent:**.
- Marketplace and gallery icon aligned with the official **Pi** brand mark.
- GitHub home moved to [YeSho-cpp/pi-coding-agent](https://github.com/YeSho-cpp/pi-coding-agent); README documents AGPL-3.0 fork lineage from FrostPi.

### Added (recent UI, still in this tree)

- Editor context chip active/inactive states; Esc / × detach; Clear wipes all chips.
- Material Icon Theme icons in QuickPick/chips when that extension is installed.
- Composer shell shortcuts `!` / `!!`, context ring, permission bar.
