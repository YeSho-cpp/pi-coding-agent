...
export const SESSION_TREE_COMMAND = "piAgent.session-tree";
export const SESSION_TREE_TOKEN_ENV = "frostui_SESSION_TREE_TOKEN";
export const SESSION_TREE_RESULT_DIR_ENV = "frostui_SESSION_TREE_RESULT_DIR";
...
export default function sessionTreeExtension(pi: ExtensionApi): void {
  pi.registerCommand(SESSION_TREE_COMMAND, {
    description: "Frost UI private session-tree adapter",
    handler: async (args, context) => {
      const request = decodeRequest(args);
      const token = requiredEnvironment(SESSION_TREE_TOKEN_ENV);
      if (request.token !== token) throw new Error("Invalid Frost UI session-tree token");
...
function decodeRequest(encoded: string): SessionTreeRequest {
  if (!encoded || encoded.length > MAX_ENCODED_REQUEST_LENGTH || !/^[A-Za-z0-9_-]+$/.test(encoded)) {
    throw new Error("Invalid Frost UI session-tree request encoding");
  }
  ...
  try {
    ...
    throw new Error("Invalid Frost UI session-tree request payload");
  }
  if (!isRecord(value)) throw new Error("Invalid Frost UI session-tree request payload");
  ...
  if (Object.keys(value).some((key) => !allowedKeys.has(key))) {
    throw new Error("Invalid Frost UI session-tree request fields");
  }
  if (
...
async function writeResult(directory: string, result: SessionTreeResult): Promise<void> {
  ...
  if (Buffer.byteLength(body, "utf8") > MAX_RESULT_LENGTH) throw new Error("Frost UI session-tree result is too large");
