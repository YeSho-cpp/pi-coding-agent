    ...
    return EditorState.create({
      ...
      extensions: [
        ...
        keymap.of([
          ...
          // Copilot-style: Enter submits; Shift+Enter inserts newline. IME composition never submits.
          { key: "Shift-Enter", run: insertPromptNewline },
          {
            key: "Enter",
            run: (view) => {
              if (view.composing) return false;
              if (acceptCompletion(view)) return true;
              onsubmit(currentStreamingBehavior);
              return true;
            },
          },
          ...historyKeymap,
        ...
        EditorView.theme({
          ...
          ".cm-scroller": {
            fontFamily: "var(--vscode-font-family, var(--font-ui))",
...
<script lang="ts" module>
  function completionExtension(commands: RpcCommandDescriptor[], sessionId: string) {
    return autocompletion({
      ...
      optionClass: (completion) => completion.type === "frostui-status" ? "frostui-completion-status" : "",
