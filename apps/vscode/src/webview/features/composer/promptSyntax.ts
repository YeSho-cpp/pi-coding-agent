...
export interface PromptToken {
  from: number;
  to: number;
  kind: "command" | "unknown-command" | "file-mention" | "shell-bang" | "shell-bang-quiet";
}
...
export function findPromptTokens(text: string, commands: ReadonlySet<string>): PromptToken[] {
  ...
  // Pi user bash: !cmd (in context) / !!cmd (exclude from context)
  const bangMatch = text.match(/^\s*(!!|!)/);
  if (bangMatch?.index !== undefined && bangMatch[1]) {
    const bang = bangMatch[1];
    const offset = bangMatch[0].indexOf(bang);
    tokens.push({
      from: offset,
      to: offset + bang.length,
      kind: bang === "!!" ? "shell-bang-quiet" : "shell-bang",
    });
  }
...
function decorateLine(from: number, text: string, commands: ReadonlySet<string>, builder: RangeSetBuilder<Decoration>): void {
  for (const token of findPromptTokens(text, commands)) {
    const className = token.kind === "command"
      ...
      : token.kind === "shell-bang"
      ? "cm-prompt-shell-bang"
      : token.kind === "shell-bang-quiet"
      ? "cm-prompt-shell-bang-quiet"
      : "cm-prompt-mention";
