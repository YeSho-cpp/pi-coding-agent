<script lang="ts">
  ...
  import ShellResultCard from "./ShellResultCard.svelte";

  let { message }: { message: CustomMessageView } = $props();
  const isShell = $derived(message.customType === "frostui-shell");
  let expanded = $state(true);

  const shellCommand = $derived(
    isShell
      ? (message.blocks.find((b) => b.type === "text") as { text: string } | undefined)?.text ?? ""
      : "",
  );
  const shellOutput = $derived(
    isShell
      ? message.blocks.filter((b) => b.type === "text").slice(1)
...
{#if isShell}
  <ShellResultCard command={shellCommand} output={shellOutput} />
{:else}
