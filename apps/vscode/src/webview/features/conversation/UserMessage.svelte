<script lang="ts">
  ...
  async function saveImage(): Promise<void> {
    ...
    try {
      ...
      await saveMessageElementAsImage(rootEl.querySelector<HTMLElement>(".user-bubble") || rootEl, {
        fileName: `frost-ui-user-${stamp}`,
      });
    } finally {
      saving = false;
    }
...
<article class="message message-user" data-pi-entry-id={message.sourceEntryId} bind:this={rootEl}>
  <div class="user-message-stack">
    <div class="user-bubble" data-message-snapshot>
    ...
    {#if copyText || message.sourceEntryId}
      <div class="message-actions">
        ...
        {#if copyText}
          ...
          <button
            type="button"
            aria-label="Save user message as image"
            title="保存为图片 (OpenChamber style DOM snapshot)"
            disabled={saving}
        ...
        {#if message.sourceEntryId}
          <button
            ...
            title={session.sessionTreeAvailable
              ...
              : "Session tree navigation is unavailable. Update Pi, restart the session, and check Pi diagnostics."}
