...
export async function configurePiExecutable(): Promise<void> {
  const configuration = vscode.workspace.getConfiguration("piAgent");
  ...
  void vscode.window.showInformationMessage("Pi executable setting updated. Restart failed sessions to apply it.");
}
