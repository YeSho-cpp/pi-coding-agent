...
import type { FrostUiConfiguration } from "./configurationTypes.js";

export function readConfiguration(scope?: vscode.Uri): FrostUiConfiguration {
  const config = vscode.workspace.getConfiguration("piAgent", scope);
  ...
  return {
    ...
    streamingBehavior: config.get<"steer" | "followUp">("composer.streamingBehavior", "steer"),
    ...
    agentPermissionMode: config.get<"ask" | "autoConfirm" | "restricted">("agent.permissionMode", "ask"),
    sessionAutoRename: config.get<boolean>("session.autoRename", true),
