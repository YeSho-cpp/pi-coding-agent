...
export class ConversationProjection {
  ...
  /** Composer `!` / `!!` result — OpenChamber-style shell card (copy command / output). */
  appendShellResult(
    command: string,
    output: string,
    ...
    const message: CustomMessageView = {
      id: `shell-${timestamp}-${++this.#sequence}`,
      type: "customMessage",
      customType: "frostui-shell",
      blocks: [
    ...
    // Optional flag in id payload via first block prefix handled in webview by exclude flag in command string
    this.#appendLiveItem({
      ...message,
      blocks: [
        { type: "text", text: excludeFromContext ? `!!${command}` : `!${command}` },
        { type: "text", text: output || "(no output)" },
      ],
    });
    this.#markContentChanged();
