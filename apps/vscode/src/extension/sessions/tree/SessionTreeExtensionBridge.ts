...
const SESSION_TREE_COMMAND = "piAgent.session-tree";
const SESSION_TREE_TOKEN_ENV = "frostui_SESSION_TREE_TOKEN";
const SESSION_TREE_RESULT_DIR_ENV = "frostui_SESSION_TREE_RESULT_DIR";
...
export class SessionTreeExtensionBridge {
  ...
  async prepare(): Promise<void> {
    this.#resultDirectory ??= await mkdtemp(join(tmpdir(), "frostui-session-tree-"));
  }
  ...
  async navigate(
    ...
    if (!this.#commandName || !this.#resultDirectory) {
      throw new Error("Session tree navigation is unavailable in this Pi process. Update Pi, restart the session, and check Pi diagnostics.");
    }
