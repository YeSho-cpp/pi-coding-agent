...
/**
 * Pi TUI composer prefixes (aligned with pi interactive-mode):
 * - `!cmd`  → run bash; output stays in agent context
 * - `!!cmd` → run bash; output excluded from context
 */
export function parseUserBashInput(text: string):
  | { command: string; excludeFromContext: boolean; display: string }
  | undefined {
  const trimmed = text.trim();
  ...
  return {
    command,
    excludeFromContext,
    display: excludeFromContext ? `!!${command}` : `!${command}`,
  };
}
