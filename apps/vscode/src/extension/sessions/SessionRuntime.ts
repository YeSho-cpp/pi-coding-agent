...
import type { FrostUiConfiguration } from "../configuration/configurationTypes.js";
...
import { commandName, normalizePiSlashPrompt, parseUserBashInput } from "./normalizePiSlashPrompt.js";
import { deriveSessionTitle, isDefaultSessionTitle } from "./autoSessionTitle.js";
...
export class SessionRuntime {
  ...
  readonly #configurationProvider: () => FrostUiConfiguration;
  ...
  constructor(
    ...
    readonly cwd: string,
    title: string,
    configurationProvider: () => FrostUiConfiguration,
  ...
  async sendPrompt(text: string, images: WebviewImageInput[], requestedStreamingBehavior?: StreamingBehavior): Promise<void> {
    ...
    // Composer `!` / `!!` → Pi user bash (same as Pi TUI). `!!` excludes output from model context.
    // Not an LLM turn: run bash, show it in the transcript, complete the turn immediately.
    const bash = parseUserBashInput(message);
    if (bash) {
      const turnId = this.#conversation.appendUserPrompt(bash.display, []);
      this.#notifyChange();
      try {
        const result = await api.bash(bash.command, { excludeFromContext: bash.excludeFromContext });
        const output = formatBashRpcResult(result);
        this.#conversation.appendShellResult(bash.command, output || "(no output)", bash.excludeFromContext);
        this.#conversation.completeTurn(turnId, "completed");
        this.#notifyChange();
      } catch (error) {
        this.#conversation.appendNotice(errorMessage(error), "error");
        this.#conversation.completeTurn(turnId, "error");
        this.#notifyChange();
        throw error;
      }
      return;
    ...
    // Auto-name new session from first user prompt (once).
    await this.#maybeAutoRenameFromPrompt(message);
  ...
  async listBranchEnds(branchPointId: string | null): Promise<BranchEndChoiceProjection[]> {
    if (!this.#sessionTreeBridge?.available) throw new Error("Session tree navigation is unavailable in this Pi process. Update Pi, restart the session, and check Pi diagnostics.");
  ...
  /** Auto title from first prompt; skips manual renames and non-default titles. */
  async #maybeAutoRenameFromPrompt(message: string): Promise<void> {
    try {
      if (this.#autoNamed) return;
      if (!this.#configurationProvider().sessionAutoRename) return;
      if (!isDefaultSessionTitle(this.view.title)) {
        this.#autoNamed = true;
        return;
      }
      ...
      if (!title) return;
      this.#autoNamed = true;
      await this.#requireApi().setSessionName(title);
      this.#viewState.setTitle(title);
      this.#notifyChange();
    } catch (error) {
      this.#logger.error("Auto session rename failed", error);
    }
  }
  ...
  async respondQuestion(requestId: string, response: QuestionDraftSubmission | { cancelled: true }): Promise<void> {
    ...
    if (!this.#questionToolBridge) throw new Error("Pi Coding Agent Question tool is unavailable.");
  ...
  refreshConfigurationState(forceRestartRequired = false): void {
    ...
    this.#viewState.setAgentPermissionMode(configuration.agentPermissionMode);
  ...
  async #startInternal(sessionFile: string | undefined, lifecycleVersion: number): Promise<void> {
    ...
    const args = [
      ...
      ...(configuration.agentPermissionMode === "restricted"
    ...
    const frostuiExtension = vscode.extensions.getExtension("yesho.pi-coding-agent");
    const frostuiVersion = (frostuiExtension?.packageJSON as { version?: string } | undefined)?.version ?? "unknown";
    const connection = new PiRpcConnection({
      cwd: this.cwd,
      args,
      env: {
        PI_INSIDE_frostui: "1",
        PI_INSIDE_frostui_VERSION: frostuiVersion,
    ...
    }, () => this.#configurationProvider().agentPermissionMode);
  ...
  async #handleQuestionUiRequest(request: RpcExtensionUiRequest): Promise<void> {
    ...
    try {
      ...
      this.#logger.error("Rejected Pi Coding Agent Question request", error);
      this.#conversation.appendNotice(`Unable to open Pi Coding Agent Question UI: ${errorMessage(error)}`, "error");
      this.#notifyChange();
...
function formatBashRpcResult(result: unknown): string {
  ...
  // Pi bash results vary: stdout/output/content/result.output...
  const stdout = pick("stdout", "output", "content", "text");
  const stderr = pick("stderr", "error", "errorMessage");
  const nested = r.result;
  if (!stdout && nested && typeof nested === "object") {
    const n = nested as Record<string, unknown>;
    if (typeof n.output === "string") parts.push(n.output);
    if (typeof n.stdout === "string") parts.push(n.stdout);
  }
  if (stdout) parts.push(stdout);
  if (stderr && stderr !== stdout) parts.push(stderr);
  const exit = r.exitCode ?? r.code ?? r.exit_code;
  if (typeof exit === "number" && exit !== 0) parts.push(`exit ${exit}`);
  if (!parts.length) {
    try {
      parts.push(JSON.stringify(result).slice(0, 4000));
    } catch { /* ignore */ }
  }
  return parts.join("\n").replace(/\n{3,}/g, "\n\n").trim().slice(0, 12000);
}
...
function extensionCommandCompletionUnconfirmedMessage(commandName: string): string {
  return `Pi Coding Agent has not confirmed that /${commandName} completed. Pi may still be waiting for input or may finish later; the session is still running.`;
}
