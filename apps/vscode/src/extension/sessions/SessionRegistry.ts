...
export class SessionRegistry implements vscode.Disposable {
  ...
  /** Throttled scan for sidebar attach — no unbounded emit loop. */
  refreshWelcomeLists(): void {
    const now = Date.now();
    if (now - this.#welcomeScanLastMs < WELCOME_SCAN_INTERVAL_MS) return;
    this.#welcomeScanLastMs = now;
    void this.refreshCatalogSessions();
    void this.#scanWelcomeResourcesQuiet();
  }

  async #scanWelcomeResourcesQuiet(): Promise<void> {
    const prevSkills = this.#welcomeSkills.join("\n");
    const prevExt = this.#welcomeExtensions.join("\n");
    await this.refreshWelcomeResources();
    if (
      this.#welcomeSkills.join("\n") !== prevSkills
      || this.#welcomeExtensions.join("\n") !== prevExt
    ) {
      this.#emitChange();
    }
  }
  ...
  async refreshCatalogSessions(): Promise<void> {
    ...
    try {
      ...
      this.#catalogSessions = [...byPath.values()]
        ...
        .slice(0, 40);
      const nextKey = this.#catalogSessions
        .map((s) => `${s.path}|${s.title}|${s.updatedAt}`)
        .join("\n");
      if (nextKey !== prevKey) this.#emitChange();
    } catch {
  ...
  async createSessionWithCustomArguments(): Promise<string | undefined> {
    return this.#withSessionCreation(async () => {
      const cwd = activeWorkspaceFolder()?.uri.fsPath;
      if (!cwd) throw new Error("Open a workspace folder before creating a Pi session.");
      // Deliberately no validation or sanitization of these arguments anywhere in Pi Coding Agent;
  ...
  async #validateRuntimeWorkingDirectory(runtime: SessionRuntime): Promise<boolean> {
    ...
    await this.#persist();
    this.#logger.info("Removed a Pi session record for a deleted worktree before process start.");
  ...
  #handleRuntimeChange(runtime: SessionRuntime): void {
    ...
    if (runtime.id !== this.#activeSessionId && inputRequired) {
      this.#toastEmitter.fire({ level: "info", message: "A background Pi session is waiting for input." });
    }
    ...
    if (runtime.id === this.#activeSessionId) {
      void vscode.commands.executeCommand("setContext", "piAgent.sessionRunning", view.isStreaming);
    }
    this.#scheduleChange();
  }
  ...
  #showSystemNotification(runtime: SessionRuntime, event: SystemNotificationEvent): void {
    ...
    const message = event === "inputRequired"
      ? backgroundSession ? "A background Pi session is waiting for input." : "Pi is waiting for your input."
      : event === "completed"
        ? backgroundSession ? "A background Pi session completed an agent turn." : "Pi completed an agent turn."
        : "A Pi session failed. Open VS Code for details.";
    const title = event === "completed" ? "Pi turn completed" : "Pi needs your attention";
  ...
  #requireActiveId(): string {
    if (!this.#activeSessionId) throw new Error("No active Pi session");
    return this.#activeSessionId;
  }
}
  ...
  const choice = await vscode.window.showWarningMessage(
    ephemeralWithPrompt
      ...
      : view.pendingExtensionUi.length > 0
        ? "Closing this Pi session will cancel its pending user interaction and stop its Pi process. Persisted Pi history is retained."
        : "Closing this Pi session will stop its active response and tools. Persisted Pi history is retained.",
    { modal: true },
    "Close session",
  );
...
async function confirmRestart(runtimes: readonly SessionRuntime[]): Promise<boolean> {
  ...
  const choice = await vscode.window.showWarningMessage(
    runtimes.length > 1
      ? "Restarting all Pi sessions will stop active responses, tools, and pending extension UI requests. Persisted Pi history is retained."
      : "Restarting this Pi session will stop its active response, tools, and pending extension UI requests. Persisted Pi history is retained.",
    { modal: true },
