...
When Pi supplies a timeout, Pi owns the default resolution. Frost UI removes the card at expiry and sends no late cancellation or value. Stopping/closing a session is different: Frost UI explicitly sends `{ cancelled: true }` for each still-pending request before terminating the process.

Frost UI never auto-confirms a request. Multiple blocking requests may be pending in one session; the coordinator retains each by RPC request id and sends at most one response for each.

## Private Question specialization

When `piAgent.questionTool.enabled` is applied at Pi process start, Frost UI injects its bundled `question` extension. The extension publishes a bounded JSON request in a per-runtime temporary directory, then calls `ctx.ui.input()` with a versioned marker containing a random per-runtime token and request id. The Extension Host handles an input as a Question request only after the marker, token, request-file identity, size, and schema all validate. Other `input` requests retain the standard card behavior.
...
Arbitrary custom TUI components, custom headers/footers/editors, raw terminal input listeners, and theme APIs are not emulated. Unknown structured methods are ignored rather than rendered from arbitrary JSON. This boundary prevents Frost UI from becoming a terminal UI runtime or executing extension-provided presentation code in the Webview.
