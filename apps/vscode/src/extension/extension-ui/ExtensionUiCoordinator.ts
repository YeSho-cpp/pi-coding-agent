...
export class ExtensionUiCoordinator {
  ...
  /** Live permission mode; ask keeps pending cards, autoConfirm answers confirm dialogs. */
  #permissionMode: () => "ask" | "autoConfirm" | "restricted";

  constructor(
    api: PiRpcApi,
    effects: ExtensionUiEffects,
    permissionMode: () => "ask" | "autoConfirm" | "restricted" = () => "ask",
  ) {
    this.#api = api;
  ...
  setPermissionModeSource(fn: () => "ask" | "autoConfirm" | "restricted"): void {
    this.#permissionMode = fn;
  }
  ...
  handle(request: RpcExtensionUiRequest): void {
    if (DIALOG_METHODS.has(request.method)) {
      ...
      // autoConfirm: only confirm dialogs — never auto-answer select/input/editor.
      if (method === "confirm" && this.#permissionMode() === "autoConfirm") {
        void this.#api.sendExtensionUiResponse(request.id, { confirmed: true });
        this.#effects.onNotify("info", `Auto-approved: ${sanitizeExtensionUiText(request.title || "Pi confirmation")}`);
        return;
      }
      this.addPending({
        id: request.id,
