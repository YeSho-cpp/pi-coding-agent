...
export async function configureProxy(registry: ProxyConfigurationActions, secrets: ProxySecretStore): Promise<void> {
  ...
  if (!scope) return;
  const configuration = vscode.workspace.getConfiguration("piAgent", scope.uri);
  ...
  const selected = await vscode.window.showQuickPick(modes, {
    title: `Pi network proxy · ${scope.label}`,
  ...
  if (selected.mode === "custom") {
    ...
    if (!vscodeProxy) {
      const action = await vscode.window.showWarningMessage(
        "VS Code http.proxy is empty. Pi will not inject a proxy until that setting is configured.",
  ...
  const action = await vscode.window.showInformationMessage(
    `Pi proxy mode set to ${proxyModeTitle(selected.mode)}. Running Pi sessions must restart before it takes effect.`,
    "Restart current",
    "Restart all",
    "Later",
  );
...
async function chooseProxyScope(): Promise<{ label: string; target: vscode.ConfigurationTarget; uri?: vscode.Uri } | undefined> {
  ...
  const selected = await vscode.window.showQuickPick(
    ...
    { title: "Where should Pi store this proxy configuration?", ignoreFocusOut: true },
  );
  return selected;
}
...
export async function configureProxyCredentials(
  ...
  if (!username) {
    ...
    registry.refreshConfigurationState(true);
    if (notify) void vscode.window.showInformationMessage("Pi proxy credentials cleared. Restart running sessions to apply.");
    return;
  }
  ...
  registry.refreshConfigurationState(true);
  if (notify) void vscode.window.showInformationMessage("Pi proxy credentials updated. Restart running sessions to apply.");
}
