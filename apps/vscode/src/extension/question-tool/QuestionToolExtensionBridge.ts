...
export class QuestionToolExtensionBridge {
  ...
  async prepare(): Promise<void> {
    this.#requestDirectory ??= await mkdtemp(join(tmpdir(), "frostui-question-tool-"));
  }
  ...
  recognizes(request: RpcExtensionUiRequest): boolean {
    return request.method === "input" && request.title?.startsWith("frostui_QUESTION:") === true;
  }
  ...
  async resolve(request: RpcExtensionUiRequest): Promise<PendingQuestionUiView> {
    ...
    if (!marker || marker.token !== this.#token) throw new Error("Invalid Pi Coding Agent Question request marker");

    const requestPath = this.#requestPath(marker.requestId);
    const info = await stat(requestPath);
    if (!info.isFile() || info.size > MAX_QUESTION_REQUEST_BYTES) throw new Error("Invalid Pi Coding Agent Question request file");
    const body = await readFile(requestPath, "utf8");
    if (Buffer.byteLength(body, "utf8") > MAX_QUESTION_REQUEST_BYTES) throw new Error("Pi Coding Agent Question request is too large");
    const value = parseQuestionRequestFile(JSON.parse(body));
    if (value.token !== marker.token || value.requestId !== marker.requestId) {
      throw new Error("Pi Coding Agent Question request identity does not match its marker");
    }
    return {
      id: request.id,
      method: "question",
  ...
  responseValue(request: PendingQuestionUiView, submission: QuestionDraftSubmission): string {
    ...
    if (Buffer.byteLength(value, "utf8") > MAX_QUESTION_RESPONSE_BYTES) throw new Error("Pi Coding Agent Question response is too large");
    return value;
  }
  ...
  #requestPath(requestId: string): string {
    ...
    if (dirname(path) !== directory || basename(path) !== `${requestId}.json`) throw new Error("Invalid Pi Coding Agent Question request path");
    return path;
  }
}
