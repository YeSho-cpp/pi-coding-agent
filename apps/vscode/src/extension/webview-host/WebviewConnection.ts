...
export class WebviewConnection implements vscode.Disposable {
    ...
    this.fileSearch = new WorkspaceFileSearch({
      onLegacyFd: (fd) => {
        void vscode.window.showWarningMessage(
          `Pi Coding Agent found fd ${fd.version}. File completion remains available, but directory suggestions require fd 10.0.0 or newer.`,
        );
      },
    });
  ...
  #resetCache(session: SessionViewModel | null): void {
    ...
    this.#outbound = this.#outbound.then(work, work).catch((error) => {
      this.#dirty = true;
      this.#logger.error("Failed to synchronize Pi Webview", error);
    });
  }
