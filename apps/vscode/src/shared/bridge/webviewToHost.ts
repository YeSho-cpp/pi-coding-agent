...
const payloadSchema = z.discriminatedUnion("type", [
  ...
  z.object({
    type: z.literal("renameCatalogSession"),
    path: z.string().min(1).max(4096),
    name: z.string().min(1).max(160),
  }),
  z.object({
    type: z.literal("deleteCatalogSession"),
    path: z.string().min(1).max(4096),
  }),
  ...
  z.object({
    type: z.literal("captureContext"),
    kind: z.enum(["file", "selection"]),
  }),
  z.object({
    type: z.literal("setEditorChipAttached"),
    attached: z.boolean(),
  }),
  z.object({
    type: z.literal("pickContext"),
    mode: z.enum(["filesAndFolders", "folder", "files"]),
  }),
  openFileSchema,
  ...
  z.object({
    type: z.literal("setStreamingBehavior"),
  ...
  z.object({
    type: z.literal("openShellInTerminal"),
    command: z.string().min(1).max(4000),
  }),
  z.object({
    type: z.literal("setAgentPermissionMode"),
    mode: z.enum(["ask", "autoConfirm", "restricted"]),
  }),
