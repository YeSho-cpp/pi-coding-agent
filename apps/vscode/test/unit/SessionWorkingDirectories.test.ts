...
describe("Session working-directory discovery", () => {
  ...
  it("discovers a linked worktree from Git's real porcelain output", async () => {
    const parent = await realpath(await mkdtemp(join(tmpdir(), "frostui-git-worktrees-")));
    ...
    await execFileAsync("git", ["-c", "user.name=Frost UI Tests", "-c", "user.email=piAgent@example.invalid", "commit", "-m", "fixture"], { cwd: main });
