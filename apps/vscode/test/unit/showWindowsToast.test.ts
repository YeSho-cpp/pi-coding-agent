...
describe("Windows native notification", () => {
  ...
  it("reports PowerShell failure to the caller", async () => {
    ...
    await expect(showWindowsToast("Pi", "Waiting", { platform: "win32", runProcess })).resolves.toBe(false);
  });
  ...
  it("does not launch PowerShell outside a local Windows Extension Host", async () => {
    ...
    await expect(showWindowsToast("Pi", "Waiting", { platform: "linux", runProcess })).resolves.toBe(false);
    await expect(showWindowsToast("Pi", "Waiting", {
      platform: "win32",
