...
describe("SessionTreeExtensionBridge", () => {
  it("owns launch credentials and discovers the collision-suffixed bundled command by source path", async () => {
    const directory = await mkdtemp(join(tmpdir(), "frostui-tree-bridge-test-"));
    ...
    const commands: RpcCommandDescriptor[] = [
      command("piAgent.session-tree", join(directory, "user-extension.js")),
      command("piAgent.session-tree:1", artifactPath),
    ...
    expect(environment.FROSTPI_SESSION_TREE_RESULT_DIR).toContain("frostui-session-tree-");
    expect(bridge.discover(commands).map((item) => item.name)).toEqual(["piAgent.session-tree", "visible"]);
    expect(bridge.available).toBe(true);
    expect(bridge.commandName).toBe("piAgent.session-tree:1");
  });
  ...
  it("invokes the discovered command and validates correlated bounded metadata", async () => {
    ...
    await bridge.prepare();
    const environment = bridge.launchEnvironment();
    bridge.discover([command("piAgent.session-tree:2", artifactPath)]);
    ...
    const executeExtensionCommand = vi.fn(async (name: string, encoded: string) => {
      ...
      expect(name).toBe("piAgent.session-tree:2");
  ...
  it("rejects uncorrelated results and removes its result directory on dispose", async () => {
    ...
    await bridge.prepare();
    const environment = bridge.launchEnvironment();
    bridge.discover([command("piAgent.session-tree", artifactPath)]);
