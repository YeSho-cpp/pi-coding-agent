  ...
  return {
    ...
    workspace: {
      ...
      getConfiguration: (section: string, scope?: { fsPath?: string }) => {
        if (section === "piAgent" && scope?.fsPath) testEnvironment.configurationScopes.push(scope.fsPath);
        return {
          get: (key: string, fallback: unknown) => {
            if (section === "piAgent" && key === "pi.executable") return testEnvironment.piExecutable;
            if (section === "piAgent" && key === "notifications.experimental.enabled") return testEnvironment.experimentalNotificationsEnabled;
            if (section === "piAgent" && key === "session.startOnOpen" && scope?.fsPath) {
...
describe("Frost UI session collection", () => {
  ...
  it("orchestrates Branch here and treats the selected current path as a no-op", async () => {
    const dir = await mkdtemp(join(tmpdir(), "frostui-registry-tree-"));
...
process.stdin.on("data", chunk => {
  ...
  while (input.includes("\n")) {
    ...
    if (command.type === "get_state") response.data = { model: null, thinkingLevel: "off", isStreaming: false, isCompacting: false, pendingMessageCount: 0, sessionFile, sessionId: "tree" };
      ...
      { name: "piAgent.session-tree", source: "extension", sourceInfo: { path: extensionPath, source: "local", scope: "temporary", origin: "top-level" } },
    ] };
...
`);
  ...
  it("creates a session in the worktree selected by the host picker", async () => {
    const main = await mkdtemp(join(tmpdir(), "frostui-registry-main-"));
    const linked = await mkdtemp(join(tmpdir(), "frostui-registry-linked-"));
  ...
  it("removes persisted sessions only after Git confirms their worktree is gone", async () => {
    ...
    expect(logger.info).toHaveBeenCalledWith("Removed 1 Frost UI session record(s) for deleted worktrees.");
  });
  ...
  it("rejects prompts while a resumed conversation history is loading", async () => {
    const dir = await mkdtemp(join(tmpdir(), "frostui-registry-"));
...
`);
  ...
  it("keeps the original identity across cancellation, recovery, and a successful temporary Fork", async () => {
    const dir = await mkdtemp(join(tmpdir(), "frostui-registry-fork-"));
...
`);
  ...
  it("uses experimental notifications for unfocused input, completion, and failure transitions", async () => {
    const dir = await mkdtemp(join(tmpdir(), "frostui-registry-notifications-"));
    const fakePi = join(dir, "fake-pi.cjs");
    await writeFile(fakePi, String.raw`#!/usr/bin/env node
let input = "";
process.stdin.setEncoding("utf8");
...
`);
    ...
    expect(windowsToastMocks.showWindowsToast).toHaveBeenCalledWith(
      "Frost UI needs your attention",
      "Frost UI is waiting for your input.",
      {},
    );
    expect(vscodeMocks.showInformationMessage).toHaveBeenCalledWith("Frost UI is waiting for your input.");
    ...
    expect(windowsToastMocks.showWindowsToast).toHaveBeenLastCalledWith(
      "Frost UI turn completed",
      "Frost UI completed an agent turn.",
      {},
    );
    ...
    expect(windowsToastMocks.showWindowsToast).toHaveBeenLastCalledWith(
      "Frost UI needs your attention",
      "A Frost UI session failed. Open VS Code for details.",
      {},
    );
  ...
  it("routes set_editor_text by Session identity regardless of sidebar selection", async () => {
    const dir = await mkdtemp(join(tmpdir(), "frostui-registry-editor-"));
    const fakePi = join(dir, "fake-pi.cjs");
    await writeFile(fakePi, String.raw`#!/usr/bin/env node
let input = "";
process.stdin.setEncoding("utf8");
